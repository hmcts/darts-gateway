...
class ContextRegistryJwtServiceSharedTokenTest extends ContextRegistryParent {
    ...
    public void before() {
        ...
        when(tokenValidator.validate(Mockito.eq(Token.TokenExpiryEnum.DO_NOT_APPLY_EARLY_TOKEN_EXPIRY), Mockito.eq(CONTEXT_REGISTRY_TOKEN))).thenReturn(true);
        when(tokenValidator.validate(Mockito.eq(Token.TokenExpiryEnum.APPLY_EARLY_TOKEN_EXPIRY), Mockito.eq(CONTEXT_REGISTRY_TOKEN))).thenReturn(true);

        for (int i = 0; i < REGISTERED_USER_COUNT; i++) {

            when(tokenValidator.validate(Mockito.eq(Token.TokenExpiryEnum.DO_NOT_APPLY_EARLY_TOKEN_EXPIRY), Mockito.eq("test2"))).thenReturn(true);
            when(tokenValidator.validate(Mockito.eq(Token.TokenExpiryEnum.APPLY_EARLY_TOKEN_EXPIRY), Mockito.eq("test2"))).thenReturn(true);
    ...
    void testHandleRegisterExpiry(ContextRegistryClient client) throws Exception {
        when(tokenValidator.validate(Mockito.any(), Mockito.eq(CONTEXT_REGISTRY_TOKEN))).thenReturn(true);
        ...
        authenticationStub.assertWithUserNameAndPasswordHeader(client, () -> {
            ...
            when(tokenValidator.validate(Mockito.any(), Mockito.eq(CONTEXT_REGISTRY_TOKEN))).thenReturn(false);
            when(tokenValidator.validate(Mockito.any(), Mockito.eq(refreshedToken))).thenReturn(true);
            ...
            verify(tokenValidator, times(12)).validate(Mockito.any(), Mockito.eq(CONTEXT_REGISTRY_TOKEN));
        }, DEFAULT_USERNAME, DEFAULT_PASSWORD);
    }
    ...
    void testHandleLookupTokenExpired(ContextRegistryClient client) throws Exception {
        when(tokenValidator.validate(Mockito.any(), Mockito.eq(CONTEXT_REGISTRY_TOKEN))).thenReturn(true, true, true, false);
        ...
        authenticationStub.assertWithUserNameAndPasswordHeader(client, () -> {
            ...
            verify(tokenValidator, times(4)).validate(Mockito.any(), Mockito.eq(CONTEXT_REGISTRY_TOKEN));
        }, DEFAULT_USERNAME, DEFAULT_PASSWORD);
    }
