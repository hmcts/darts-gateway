...
class BasicCacheTest {
    ...
    void before() {
        ...
        Mockito.when(validateToken.validate(Mockito.eq(Token.TokenExpiryEnum.DO_NOT_APPLY_EARLY_TOKEN_EXPIRY), Mockito.notNull())).thenReturn(true);
    ...
    void testRegisterWithExisting() {
        ...
        Mockito.when(validateToken.validate(Mockito.eq(Token.TokenExpiryEnum.APPLY_EARLY_TOKEN_EXPIRY), Mockito.notNull())).thenReturn(true);
        Mockito.when(generatable.createToken(Mockito.eq(context))).thenReturn(returnToken);
        Mockito.when(generatable.getToken(Mockito.notNull())).thenReturn(returnToken);
    ...
    void testLookupWithEvictionDueToInvalidToken() {
        Mockito.when(validateToken.validate(Mockito.eq(Token.TokenExpiryEnum.DO_NOT_APPLY_EARLY_TOKEN_EXPIRY), Mockito.notNull())).thenReturn(false);
