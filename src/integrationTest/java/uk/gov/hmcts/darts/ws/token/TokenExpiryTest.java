...
public class TokenExpiryTest extends IntegrationBase {
    ...
    public void checkTokenInvalidDueToExpiry(ContextRegistryClient client) throws Exception {
        // make sure we leave enough time for the refresh between runs
        contextRequestHelper.registerToken(client, getGatewayUri());

        Assertions.assertTrue(!LogUtil.getMemoryLogger().searchLogs("JWT Token could not be validated", "Expired JWT", Level.ERROR).isEmpty());
    }
}
