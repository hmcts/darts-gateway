...
@MultipartConfig
public class DartsSOAPMessageDispatcherServlet extends MessageDispatcherServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        if (HttpMethod.GET.name().equals(req.getMethod())) {
            ...
            for (EndpointMetaData metaData : endpointMetaDataList) {
                ...
                if (query.equalsIgnoreCase("wsdl") && metaData.getEndpointMatcher().asMatchPredicate().test(req.getRequestURI())) {
                    try (InputStream wsdlInputStream = metaData.getConsolidatedWsdlStream().getInputStream()) {
                        String text = new String(wsdlInputStream.readAllBytes(), StandardCharsets.UTF_8);
