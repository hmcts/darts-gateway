...
public class DartsRequestMultiPart extends HttpServletRequestWrapper {
    ...
    private final static int MEDIA_PLUS_XML_PART_COUNT=2;

    private File payload;
    public DartsRequestMultiPart(HttpServletRequest request) {
        super(request);
        ...
        try {
            ...
            IOUtils.copy(request.getInputStream(), new FileOutputStream(payload));

            MimeMultipart mimeMultipart = new MimeMultipart(new ByteArrayDataSource(new FileInputStream(payload), "application/octet-stream"));
    ...
    private boolean getBinary(BodyPart part) throws IOException {
        try {
            return part.getHeader("Content-Type")[0].equals("application/octet-stream");
        }
    ...
    public ServletInputStream getInputStream() throws IOException {
        ...
        CachedServletInputStream stream = new CachedServletInputStream(new FileInputStream(payload));
        payload.delete();
        return stream;
    }
